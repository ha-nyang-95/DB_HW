...
# %%
# TODO 1: Prometheus API를 호출하여 특정 컨슈머 그룹의 모든 파티션 Lag 합계를 반환하는 함수를 구현하세요.
def get_consumer_lag():
    response = requests.get(PROMETHEUS_URL)  # Prometheus API 호출 함수
    lines = response.text.split('\n')
    total_lag = 0
    for line in lines:
        if f'kafka_consumergroup_lag{{consumergroup="{CONSUMER_GROUP}"' in line:  # 특정 컨슈머 그룹의 Lag 정보 필터링
...
while True:
    ...
    # TODO 2: Lag이 임계값 이상이면 경고 메시지를 출력하세요.
    if lag >= LAG_THRESHOLD:  # Lag 임계값 설정
        print("WARNING: Consumer Lag is too high!")

    time.sleep(CHECK_INTERVAL)  # TODO 3: 일정 주기마다 Lag을 체크하는 시간 간격 설정
